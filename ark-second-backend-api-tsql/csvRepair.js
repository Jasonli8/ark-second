          ...
          if (fs.existsSync(dest)) {
            ...
            file.on("line", async (line) => {
              if (lineNum !== 1) {
                ...
                const query3 =
                  `IF EXISTS (SELECT * FROM [Shares].[Company] AS [c] JOIN [Shares].[Holding] AS [h] ON [c].[Id] = [h].[companyId] JOIN [Shares].[Fund] AS [f] ON [f].[Id] = [h].[fundId] WHERE [cusip] = '${data[4]}' AND [fundName] = '${fundName}' AND [date] = ${date}) ` +
                  ...
                  `UPDATE All_Holdings SET [shares] = ${data[5]}, [marketValue] = ${data[6]}, [weight] = ${data[7]} WHERE [cusip] = '${data[4]}' AND [fundName] = '${fundName}' AND [date] = ${date} ` +
                  `ELSE WITH All_Holdings ([cusip], [fundName], [date], [shares], [marketValue], [weight]) AS (SELECT [companyName], [fundName], [date], [shares], [marketValue], [weight] FROM [Shares].[Company] AS [c] JOIN [Shares].[Holding] AS [h] ON [c].[Id] = [h].[companyId] JOIN [Shares].[Fund] AS [f] ON [f].[Id] = [h].[fundId]) ` +
                  `INSERT INTO [Shares].[Holding]([companyId], [fundId], [date], [shares], [marketvalue], [weight]) SELECT [c].[Id], ${id}, ${date}, ${data[5]}, ${data[6]}, ${data[7]} FROM [Shares].[Company] AS [c] WHERE [c].[cusip] = '${data[4]}'`;
                try {
